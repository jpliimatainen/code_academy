...
module.exports = {
    ...
    deleteCustomer: async (req, res) => {
        try {
            ...
            if (result.length > 0) { // shipped items for this customer
                // set customer status as 'not active'
                result = await customer.setCustomerStatus(customerId, 1);

                if (result.affectedRows === 0) {
                    res.statusCode = 400;
                    res.json({ status: "NOT OK", msg: "Virhe asiakkaan asettamisessa ei-aktiiviseksi." });
                    return;
                }
                else { // success
                    res.statusCode = 200;
                    res.json({ status: "OK", msg: "Asiakas asetettu ei-aktiiviseksi." });
                    return;
                }
            }
